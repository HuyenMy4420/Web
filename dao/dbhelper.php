...
function addProductToCart($loginId, $accountId, $productId, $price) {
	if(!empty($loginId) && !empty($accountId) && !empty($productId) && !empty($price)) {
		...
		if($stmt->rowCount() > 0)
		{
			echo "hii";
			//cập nhật giỏ hàng
			$stmt = $dbh->prepare("SELECT * from line_item where product_id = ? AND shopping_cartid = ? AND order_number = NULL");
			...
			if($stmt->rowCount() > 0) {
				...
				$addSucess = $dbh->prepare($sql)->execute([$resultLineItem[0]['id']]);
			}
			else {
				//tạo mới line_item
...
function getProductsInCart($cartId) {
	...
	$query = "SELECT product.id, product.name, product.price, product.image, line_item.quantity FROM product JOIN line_item ON product.id = line_item.product_id WHERE line_item.shopping_cartid = ".$cartId." AND line_item.order_number IS NULL;";
